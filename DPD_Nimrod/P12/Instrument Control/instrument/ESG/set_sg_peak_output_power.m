function set_sg_output_power(num_gen,opower,off_d_i,off_d_o,assumed_gain)
% set_sg_output_power.m
%
% PURPOSE: To find input power for the signal generators that will generate
%          the desired peak output power. Supports any number of signal generators.
% PARAMETERS: num_gen - the number of signal generators that are needed
%                       opower - Required peak output power in dBm.
%                       assumed_gain - Optional parameter. If not entered maximum assumed gain is 80 dB.

if (nargin~=4 && nargin~=5)
    help set_sg_peak_output_power
...
%opower=opower-10*log10(num_gen)+0.02-10; % Assume peak to average to be about 10.
opower_orig=opower;
...
for i=1:num_gen
    ...
    if(stat)
        ...
        power(i)=opower-assumed_gain-10; % Assume maximum gain of 80 dB and PAR of 10 dB.
...
while(1)
    ...
    for g=1:num_gen % Loop over all generators
        ...
        p_old2=0;
%         [pi_prev,po_prev]=get_sc_pkpk('CH1','CH2');
%         pi_prev=ff(1000*abs(pi_prev),pin_245_12049)+off_d_i(2,2);
%         po_prev=ff(1000*abs(po_prev),pin_245_13606)+off_d_o(2,2);
%         pg_prev=power(g);
        for i=1:20 % Maximum Newton-Raphson iterations.
            set_sg_power(g,power(g)); % Set power.
            pause(0.5);
            [p,a]=get_sc_pkpk('CH2','CH1');
            p=ff(1000*abs(p),Pin_245_13606)+off_d_o(2,2);
            d=p-opower;
            if(abs(d)<0.1) 
                break;
            end
            %%%% Sanity check - Test whether the gain change at input and output is consistent with the gain change in the generator.
%             [pi,a]=get_sc_pkpk('CH1','CH2');
%             pi=ff(1000*abs(pi),pin_245_12049)+off_d_i(2,2);
...
%             end
            if(abs(p-p_old2)<0.04) % We are probably stuck due to attenuator jump.
