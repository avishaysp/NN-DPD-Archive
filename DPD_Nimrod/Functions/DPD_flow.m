...
signal.esg_sample_rate      = 32;    % [MHz]
signal.is_CW                = 0;


model.PA_k = 3;    % PA fitting model - nonlinear 
model.PA_q = 1;     % PA fitting model - memory 

model.k = 3;        % DPD fitting model - nonlinear
model.q = 1;        % DPD fitting model - memory 
...
% Output_signal = load([measured_dir measured_pa_output], 'Y');
Output_signal = load('C:\Users\nimrodg\Documents\MATLAB\Nimrod\FD\DPD_test.mat');
Output_signal.XDelta = Output_signal.X(2)-Output_signal.X(1);
...
% Resample signals 
close all
Input_signal=packet(:,1)-1j*packet(:,2);
Input_signal= Input_signal/rms(Input_signal);
Output_signal=Y/rms(Y);
signal.signal_sample_rate = 1280;
...
if 1 % Plot spectral desity       
    ...
    legend('Ideal', 'Output w/o DPD' , ...
                    ...
                    'DPD-Floor' , ...
                    'Pre-Distort'         );
    xlim([-mutual_sample_rate  mutual_sample_rate]); 
%     ylim([-60 40])
    % RMS calculation 
